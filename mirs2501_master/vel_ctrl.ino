...
void vel_ctrl_execute() {
  ...
  // PID制御 (左右それぞれ別のゲインを使用)
  pwm_l = Kp_l * err_curr_l + Ki_l * err_sum_l + Kd_l * (err_curr_l - err_prev_l);
  pwm_r = Kp_r * err_curr_r + Ki_r * err_sum_r + Kd_r * (err_curr_r - err_prev_r);

  // 速度指令値 = 0 なら強制的に停止
  if (vel_ref_l == 0.0) { pwm_l = 0; err_sum_l = 0; } // 停止時は積分もリセット推奨
  if (vel_ref_r == 0.0) { pwm_r = 0; err_sum_r = 0; }

  motor_set(pwm_l, pwm_r);

  // 積分リセット処理（速度がほぼ0のとき）
  if(fabs(vel_curr_l) < 0.1 && fabs(vel_curr_r) < 0.1) {
    err_sum_l = 0;
    err_sum_r = 0;
  }
